...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = key_states.new_zeros(bsz, 1, tgt_len, src_len)
        ...
        attention_mask = attention_mask.reshape_as(attn_weights)

        if do_quant:
            quant_attn_weights = quant_qk_matmul(query_states, key_states, self.config, self.quant_matmul)
        else:
            quant_attn_weights = None

        if do_prune:
            attn_scores = quant_attn_weights if do_quant else attn_weights
        ...
        #         if attention_mask is None:
        #             attention_mask = 0
        #         attention_mask = attention_mask + torch.zeros((bsz, self.num_heads, tgt_len, src_len), device = key_states.device)

        #         with torch.no_grad():
        #             query_norm = query_states.norm(dim = 2, keepdim=True)
        #             key_norm = key_states.norm(dim = 1, keepdim=True)
        #             normalized_query_states = query_states/query_norm
        #             normalized_key_states = key_states/key_norm

        #         quant_attention_scores = self.quant_matmul(normalized_query_states, normalized_key_states)
        #         quant_attention_scores *= query_norm*key_norm

        #         quant_attention_scores = quant_attention_scores.view(*attention_mask.shape)
        #         sparsity_mask = prune_attn_scores(quant_attention_scores, attention_mask, self.config)
        ...
        if do_static_sparsity:
            ...
            apply_static_sparsity_mask(self.static_sparsity_mask, attn_weights)
